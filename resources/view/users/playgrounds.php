    ...
    <div id="posts-data" class="products-container row">
        ...
        if ($query->have_posts()) :
            while ($query->have_posts()) :
                $query->the_post();
                $t = '';
                // ($count == 3) ? 'hx-get="' . get_next_posts_page_link($query->max_num_pages) . '" hx-trigger="revealed"
                //     hx-swap="afterend"' : '';
        ?>
                <div class="col-12" <?php echo (get_next_posts_page_link($query->max_num_pages)) ? $t : ''; ?>>
                    <div class="mb-3 position-relative shadow p-3 rounded-2">
                        ...
                        <div class="d-grid mt-3">
                            <a hx-get="<?php echo site_url('user/playground/edit/' . $post->ID . '/'); ?>" hx-swap="innerHTML show:top" hx-trigger="click" hx-target=".app" hx-on::before-request="showLoad()" class="btn btn-danger">
